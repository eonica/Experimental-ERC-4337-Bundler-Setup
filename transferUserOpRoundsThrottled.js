...
const SIMPLE_ACCOUNT_ADDRESSES = loadSimpleAccountAddresses(SIMPLE_ACCOUNT_ADDRESSES_FILE, SCA_NUMBER);
const SCA_OWNERS_NUMBER = 10;
const SIMPLE_ACCOUNT_OWNER_KEYS = [
...
// -----------------------------
async function prepareAccounts(publicClient) {
    return await Promise.all(SIMPLE_ACCOUNT_ADDRESSES.map((addr, i) => {
...
function prepareCalldata() {
    return SIMPLE_ACCOUNT_ADDRESSES.map((_, i) => encodeFunctionData({
        abi: TestTokenAbi,
...
// -----------------------------
async function runRound(round, bundlerClient, simpleAccounts, calldatas) {
    console.log(`\n================ ROUND ${round} START ================`);
    ...
    for (let i = 0; i < simpleAccounts.length; i++) {
        ...
        const promise = (async () => {
            try {
                ...
                console.log(`[Round ${round}] UserOperation #${i + 1} sent → hash: ${userOpHash}`);
                return { i, userOpHash };
            }
            catch (error) {
                console.error(`[Round ${round}] UserOperation #${i + 1} failed to send:`, error);
    ...
    const waitPromises = sentOps.map(async ({ i, userOpHash }) => {
        ...
        try {
            ...
            console.log(`[Round ${round}] UserOperation #${i + 1} confirmed in block ${receipt.receipt.blockNumber}!`);
            return receipt;
        }
        catch (error) {
            console.error(`[Round ${round}] UserOperation #${i + 1} failed to confirm:`, error);
            return null;
        }
    });
    ...
    receipts.forEach((r, i) => {
        if (r) {
            console.log(`[Round ${round}] #${i + 1}: confirmed in block ${r.receipt.blockNumber}`);
        }
        else {
...
//# sourceMappingURL=transferUserOpRoundsThrottled.js.map
